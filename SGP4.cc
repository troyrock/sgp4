...
void SGP4::UpdateElements(double n, double e, double bstar)
{
    elements_.SetMeanMotion(n);
    elements_.SetBStar(bstar);
    // eccentricity not easily updated in one go here, keeping simple for now
    Initialise();
}

void SGP4::UpdateBStar(double bstar)
{
    /*
     * SGP4 depends on several constants derived from B*.
     * The primary one is c1.
     * We need to re-derive c1 from the new B*.
     */
    
    // We need to re-calculate common_consts_.c1 which is BStar * c2
    // We'll store c2 or re-calculate it. For simplicity, we re-calculate
    // the c2 part here based on existing common_consts_ and elements_

    const double betao2 = 1.0 - elements_.Eccentricity() * elements_.Eccentricity();
    const double psisq = fabs(1.0 - common_consts_.eta * common_consts_.eta);
    const double tsi = 1.0 / (elements_.RecoveredSemiMajorAxis() - kS); // Note: simplifies perigee < 156km check for now

    // Re-calculating c2 part of common_consts_.c1
    double s4 = kS;
    double qoms24 = kQOMS2T;
    if (elements_.Perigee() < 156.0)
    {
        s4 = elements_.Perigee() - 78.0;
        if (elements_.Perigee() < 98.0) s4 = 20.0;
        qoms24 = pow((120.0 - s4) * kAE / kXKMPER, 4.0);
        s4 = s4 / kXKMPER + kAE;
    }
    const double tsi_actual = 1.0 / (elements_.RecoveredSemiMajorAxis() - s4);
    const double coef = qoms24 * pow(tsi_actual, 4.0);
    ...
    const double c2 = coef1 * elements_.RecoveredMeanMotion()
        ...
        * (1.0 + 1.5 * etasq + eeta * (4.0 + etasq))
        + 0.75 * kCK2 * tsi_actual / psisq * common_consts_.x3thm1
        * (8.0 + 3.0 * etasq * (8.0 + etasq)));

    common_consts_.c1 = bstar * c2;
    
    // xnodcf also depends on c1
    const double temp1 = 3.0 * kCK2 * elements_.RecoveredMeanMotion() / 
        (elements_.RecoveredSemiMajorAxis() * elements_.RecoveredSemiMajorAxis() * betao2 * betao2);
    ...
    // For NearSpace, omgcof and xmcof also depend on B*
    if (!use_deep_space_) {
        double c3 = 0.0;
        if (elements_.Eccentricity() > 1.0e-4) {
            c3 = coef * tsi_actual * kA3OVK2 * elements_.RecoveredMeanMotion() * kAE *
                    common_consts_.sinio / elements_.Eccentricity();
        }
        nearspace_consts_.omgcof = bstar * c3 * cos(elements_.ArgumentPerigee());
        if (elements_.Eccentricity() > 1.0e-4) {
...
{
    along_track_bias_ = bias_rad;
}

void SGP4::DebugPrintState() const
{
    std::cout << std::fixed << std::setprecision(12);
    std::cout << "--- SGP4 INTERNAL STATE ---" << std::endl;
    std::cout << "omgdot: " << common_consts_.omgdot << std::endl;
    std::cout << "xnodot: " << common_consts_.xnodot << std::endl;
    std::cout << "xmdot:  " << common_consts_.xmdot << std::endl;
    std::cout << "c1:     " << common_consts_.c1 << std::endl;
    std::cout << "c4:     " << common_consts_.c4 << std::endl;
    std::cout << "---------------------------" << std::endl;
}
...
Eci SGP4::FindPositionSDP4(double tsince) const
{
    ...
    xl = xmam + omgadf + xnode + along_track_bias_;
    omega = omgadf;
...
Eci SGP4::FindPositionSGP4(double tsince) const
{
    ...
    xl = xmp + omega + xnode + elements_.RecoveredMeanMotion() * templ + along_track_bias_;
